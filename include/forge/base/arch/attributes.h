#line 1 "C:/Users/tyler/dev/WINGG/forge/base/arch/attributes.h"
/*
 ...
 */
#ifndef FORGE_BASE_ARCH_ATTRIBUTES_H
#define FORGE_BASE_ARCH_ATTRIBUTES_H
...
// Emit a Arch_ConstructorEntry in the __Data,forgector section.
#   define ARCH_CONSTRUCTOR(_name, _priority, ...)                             \
    static void _name(__VA_ARGS__);                                            \
    static const Arch_ConstructorEntry _ARCH_CAT_NOEXPAND(arch_ctor_, _name)   \
        __attribute__((used, section("__DATA,forgector"))) = {                 \
...
// Emit a Arch_ConstructorEntry in the __Data,forgedtor section.
#   define ARCH_DESTRUCTOR(_name, _priority, ...)                              \
    static void _name(__VA_ARGS__);                                            \
    static const Arch_ConstructorEntry _ARCH_CAT_NOEXPAND(arch_dtor_, _name)   \
        __attribute__((used, section("__DATA,forgedtor"))) = {                 \
        reinterpret_cast<Arch_ConstructorEntry::Type>(&_name),                 \
        0u,                                                                    \
        _priority                                                              \
    };                                                                         \
...
#   define ARCH_CONSTRUCTOR(_name, _priority, ...) \
        __attribute__((used, section(".forgector"), constructor((_priority) + 100))) \
        static void _name(__VA_ARGS__)
#   define ARCH_DESTRUCTOR(_name, _priority, ...) \
        __attribute__((used, section(".forgedtor"), destructor((_priority) + 100))) \
        static void _name(__VA_ARGS__)
...
#   pragma section(".forgector", read)
#   pragma section(".forgedtor", read)
...
// Emit a Arch_ConstructorEntry in the .forgector section.  The namespace and
...
#   define ARCH_CONSTRUCTOR(_name, _priority, ...)                             \
    ...
    // Emit a Arch_ConstructorEntry in the .forgedtor section.
#   define ARCH_DESTRUCTOR(_name, _priority, ...)                              \
    static void _name(__VA_ARGS__);                                            \
    namespace {                                                                \
    __declspec(allocate(".forgedtor"))                                         \
    extern const Arch_ConstructorEntry                                         \
...
#endif // FORGE_BASE_ARCH_ATTRIBUTES_H
